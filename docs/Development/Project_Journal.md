[Home](../../README.md) › [Project Index](../../PROJECT_INDEX.md) › [Development](README.md) › Project Journal

# Project Journal

> **Status:** Maintained
> **Owner:** Philippine Marching Percussion Fusion Project
> **Applies To:** Project evolution records and working observations
> **Last Reviewed:** 2026-07-13
> **Review Frequency:** As needed
> **Authoritative:** No

## Purpose

The Project Journal documents the project's evolution over time. It records working observations, experiments, and lessons learned—not authoritative specifications.

## What Belongs Here

- research findings before they mature into Reference or Research documents
- experimentation notes and pilot observations
- cadence and warm-up development history
- educational observations from field testing
- lessons learned from rehearsals and performances
- identity decisions and rationale before they are formalized as ADRs

## What Does Not Belong Here

- authoritative identity statements (see [Identity Framework](../Specifications/Philippine_Marching_Percussion_Identity_Framework.md))
- stable concept definitions (see [Concept Library](../Reference/Concept_Library.md))
- formal research write-ups (see [Research](../Research/README.md))
- finished scores and audio (see `scores/` and `audio/`)

## Information Lifecycle

```text
Journal observation or experiment
        ↓
Research note or concept draft
        ↓
Specification, Reference, or User Guide
        ↓
Score or audio deliverable
```

## Entries

Journal entries will be added below as dated sections or linked files as the project progresses.

### 2026-07-13 — Instructor documents for MONHS pilot

Added [Instructor Introduction](../User_Guides/Instructor_Introduction.md) and [MONHS 2026 Pilot Brief](../User_Guides/MONHS_2026_Pilot_Brief.md) for music educator onboarding.

### 2026-07-13 — Musical Identity Foundation

Design-phase decisions from the project's identity foundation work were distributed into canonical EDF locations:

- [Philippine Marching Percussion Identity Framework](../Specifications/Philippine_Marching_Percussion_Identity_Framework.md)
- [Design Principles](../Specifications/Design_Principles.md)
- [Concept Library](../Reference/Concept_Library.md) (expanded)
- [Research backlog](../../tasks/research-backlog.md)

## Related Documents

- [Development Overview](README.md)
- [Active Tasks](../../tasks/README.md)
- [Research](../Research/README.md)
