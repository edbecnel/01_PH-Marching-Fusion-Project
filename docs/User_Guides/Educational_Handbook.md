[Home](../../README.md) › [Project Index](../../PROJECT_INDEX.md) › [User Guides](README.md) › Educational Handbook

# Educational Handbook

> **Status:** Draft
> **Owner:** Philippine Marching Percussion Fusion Project
> **Applies To:** Educators, section leaders, and pilot instructors
> **Last Reviewed:** 2026-07-13
> **Review Frequency:** Quarterly
> **Authoritative:** Yes

## Purpose

This handbook explains how to teach the ideas defined in the [Identity Framework](../Specifications/Philippine_Marching_Percussion_Identity_Framework.md) and [Design Principles](../Specifications/Design_Principles.md). It is practical instruction, not identity definition.

New educators should begin with the [Instructor Introduction](Instructor_Introduction.md).

## Teaching Responsibilities

This handbook will explain how to:

- study indigenous traditions as cultural context
- study festival drumming as rhythmic and performance foundation
- teach groove before speed
- integrate movement with music
- build cultural understanding alongside technique
- run rehearsal methodology suited to Philippine school environments
- support progressive student development from rote learning toward blended literacy

## Pedagogical Foundation

The project introduces notation as an additional tool rather than a replacement for learning by ear. Traditional local lines develop listening, memory, feel, and cultural continuity. Written materials add repeatability, preservation, and advanced rhythmic organization.

See [Core Blueprint — Pedagogy](../Specifications/Core_Blueprint.md#3-pedagogy-from-rote-learning-to-blended-literacy).

## Hybrid Warm-Up System

Technical exercises combine marching-percussion fundamentals with Philippine rhythmic vocabulary. Initial subjects include [legatos](../Reference/Legato_Stroke.md), double strokes, [accent-taps](../Reference/Accent_Taps.md), [8-8-16](../Reference/Eight_Eight_Sixteen.md), flams, and [grid exercises](../Reference/Grid_Exercises.md).

DCI stick-height control is taught through accent groupings influenced by Philippine festival rhythms.

## Related Documents

- [Instructor Introduction](Instructor_Introduction.md)
- [Legato Stroke](../Reference/Legato_Stroke.md)
- [Accent-Taps](../Reference/Accent_Taps.md)
- [8-8-16 Exercise](../Reference/Eight_Eight_Sixteen.md)
- [Grid Exercises](../Reference/Grid_Exercises.md)
- [Philippine Marching Percussion Identity Framework](../Specifications/Philippine_Marching_Percussion_Identity_Framework.md)
- [Design Principles](../Specifications/Design_Principles.md)
- [Arranging Guide](Arranging_Guide.md)
- [Technique and Warm-ups](../../../scores/technique-and-warmups/README.md)
- [Audio — Battery Splits](../../../audio/battery-splits/README.md)

## Status

Detailed lesson plans and teaching sequences are pending development. This document establishes scope and canonical references.
